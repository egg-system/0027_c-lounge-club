...
class SwpmAuth {
    ...
    private function check_constraints() {
        ...
        //会員レベルチェック
        $query = "SELECT * FROM " . $wpdb->prefix . "swpm_membership_tbl WHERE id = %s";
        $levelData = $wpdb->get_row($wpdb->prepare($query, $this->userData->membership_level));
        if ($levelData->role != 'subscriber') {
          $this->lastStatusMsg = SwpmUtils::_('月額費用の決済が行われていないため、ログインできません。<br/>登録したメールをご確認いただき決済の手続きを行うか、運営事務局までお問い合わせください。');
          $this->isLoggedIn = false;
          $this->userData = null;
          return false;
        }
