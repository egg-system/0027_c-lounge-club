...
class SwpmAccessControl {
    ...
    public function can_i_read_post($post){
        ...
        if(!$auth->is_logged_in()){
            ...
            $text = SwpmUtils::_('ここから先は会員専用ページです。会員の方はログインをしてください。<br>') . SwpmMiscUtils::get_login_link();
    ...
    public function can_i_read_comment($comment){
        ...
        if(!$auth->is_logged_in()){
            ...
            $error_msg = '<div class="swpm-comment-not-logged-in">' . SwpmUtils::_("You need to login to view this content. <br>") . '</div>';
