...
class SwpmFbFieldmeta {
    ...
    private function verification($value, $form_type = 0) {
        ...
        // Only display verification if there verification field in the form
          ...
          </li>', $verification_field_legend, $this->sequence, __('Please enter any two digits with <strong>no</strong> spaces (Example: 12)', 'swpm')
          );
          }
    ...
    private function password($form_type = 0, $label_align = "") {
        ...
        $form_item = sprintf(
                '<div><input type="password" placeholder="' . __('Type password Here', 'swpm') . '" name="swpm-%1$d" id="%2$s" value="%3$s" class="swpm-text %4$s %5$s %6$s %7$s" /></div>', $field_id, $id_attr, $default, $size, $required, $validation, $css
        );
        $form_item .= sprintf(
                '<div><input type="password" placeholder="' . __('Retype password Here', 'swpm') . '" name="swpm-%1$d_re" id="%2$s" value="%3$s" class="swpm-text %4$s %5$s %6$s %7$s" /></div>', $field_id, $id_attr, $default, $size, $required, $validation, $css
        );
    ...
    private function checkbox($value = null, $form_type = 0, $label_align = "") {
        ...
        foreach ($field_options as $option => $value) {
            $option++;
            $options .= sprintf(
                    '<div class="swpm-span"><input type="checkbox" name="swpm-%1$d[%3$d]" id="%2$s-%3$d" value="%5$s" class="swpm-checkbox %4$s"%7$s /><label for="%2$s-%3$d" class="swpm-choice">%6$s</label></div>', $field_id, $id_attr, $option, $css, esc_attr(trim(stripslashes($option))), wp_specialchars_decode(stripslashes($value)), checked(in_array($option, $values) ? $option : -1, $option, 0)
            );
        }
    ...
    private function address($value = array(), $form_type = 0, $label_align = "") {
        ...
        $address_parts = array(
            'address' => array(
                'label' => __('Street Address', 'swpm'),
                'layout' => 'full',
                'value' => isset($value['address']) ? $value['address'] : ''
            ),
            'address-2' => array(
                'label' => __('Apt, Suite, Bldg. (optional)', 'swpm'),
                'layout' => 'full',
            ...
            'state' => array(
                'label' => __('State / Province / Region', 'swpm'),
                'layout' => 'right',
                'value' => isset($value['state']) ? $value['state'] : ''
            ),
            'zip' => array(
                'label' => __('Postal / Zip Code', 'swpm'),
                'layout' => 'left',
                'value' => isset($value['zip']) ? $value['zip'] : ''
            ),
            'country' => array(
                'label' => __('Country', 'swpm'),
                'layout' => 'right',
    ...
    private function date($value = null, $form_type = 0, $label_align = "") {
        ...
        $value = empty($value) ? $default : $value;
        $form_item_format = '<input type="text" name="swpm-%1$d" id="%2$s" value="%3$s" class="swpm-text swpm-date-picker %4$s %5$s %6$s" data-dp-dateFormat="%7$s" />';
        $form_item = sprintf($form_item_format, $field_id, $id_attr, $value, $size, $required, $css, $dateFormat);
    ...
    private function time($value = null, $form_type = 0, $label_align = "") {
        ...
        // AM/PM
        $ampm_format = '<span class="swpm-time"><select name="swpm-%1$d[ampm]" id="%2$s-ampm" class="swpm-select %5$s %6$s"><option value="AM">AM</option><option value="PM">PM</option></select><label for="%2$s-ampm">AM/PM</label></span>';
        if ($time_format == '12') {
