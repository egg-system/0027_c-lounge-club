...
class Swpm_Form_Builder {
    ...
    public function get_member_field_by_id($output, $id, $field) {
        ...
        $sql = $wpdb->prepare("SELECT  `value`
               ...
               WHERE C.user_id =%d AND F.field_name = %s", $id, $field);
    ...
    public function init() {
        ...
        $swpm = 'simple_wp_membership';
        $page = filter_input(INPUT_GET, 'page');
        if (is_admin() && substr($page, 0, strlen($swpm)) == $page) {
            $this->admin_scripts();
        } else {
            $this->scripts();
        }
    ...
    private function create_field($form_data, $reg_field_id = 0) {
        ...
        if (!empty($insert_id)) {
            ...
            if (!$sequence_last_row) {
                ...
                $sequence_last_row = $wpdb->get_var(
                        $wpdb->prepare(
                                "SELECT field_sequence FROM $this->field_table_name WHERE form_id = %d AND field_type = 'submit' ORDER BY field_sequence DESC LIMIT 1", $form_id));
            }
    ...
    function confirmation_text() {
        ...
        foreach ($forms as $form) :
            ...
            } else if ($form->form_type == SwpmFbFormCustom::REGISTRATION) {
                return SwpmUtils::_('Registration successful.');
            } else {
